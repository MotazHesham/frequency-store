...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.frequancy.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.frequancies.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="frequency">{{ trans('cruds.frequancy.fields.frequency') }}</label>
                <input class="form-control {{ $errors->has('frequency') ? 'is-invalid' : '' }}" type="number" name="frequency" id="frequency" value="{{ old('frequency', '') }}" step="0.001" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('frequency_type') ? 'is-invalid' : '' }}" name="frequency_type" id="frequency_type" required>
                    ...
                    @foreach(App\Models\Frequancy::FREQUENCY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('frequency_type', 'vhf') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tag_1') ? 'is-invalid' : '' }}" type="text" name="tag_1" id="tag_1" value="{{ old('tag_1', '') }}" required>
                @if($errors->has('tag_1'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tag_1') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tag_2') ? 'is-invalid' : '' }}" type="text" name="tag_2" id="tag_2" value="{{ old('tag_2', '') }}">
                @if($errors->has('tag_2'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tag_2') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tag_3') ? 'is-invalid' : '' }}" type="text" name="tag_3" id="tag_3" value="{{ old('tag_3', '') }}">
                @if($errors->has('tag_3'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tag_3') }}
                    </div>
